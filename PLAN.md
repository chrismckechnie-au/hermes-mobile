...
`GET /v1/models` lists the Host default plus configured `model_routes` aliases (`api_server.py:1421-1455`); `POST /v1/runs` accepts a `model` field resolved per request — unknown aliases fall back to the Host default, and a session `/model` override always wins (`api_server.py:1207-1232`). Client: fetch models on connect, show a model chip in the chat header that opens a Model settings sheet (model + reasoning dropdowns), pass the selection as `model` on each run submission. Selection is in-memory per Host (no persistence — the Host default is always a safe fallback).

**Reasoning selection (second addition)** required a small Host patch — the API had no per-request reasoning hook (effort was global `agent.reasoning_effort` config; `model_routes` can't carry it). Patch: `POST /v1/runs` accepts optional `reasoning_effort` (validated via `hermes_constants.parse_reasoning_effort`, 400 on invalid), `_create_agent` takes a per-run `reasoning_override`, and capabilities advertise `run_reasoning_effort`. The client gates the selector on that flag, so a hermes-agent update that reverts the patch just hides the control instead of breaking sends. Host `model_routes` for the ChatGPT models were also added to `config.yaml` under `gateway.platforms.api_server.extra`.
